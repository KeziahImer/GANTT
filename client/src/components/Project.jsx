...
const Project = () => {
  ...
  const getProject = async () => {
    try {
      const res = await axios.get(
        `http://localhost:8000/api/projects/details/${userId}`
      );
  ...
  const printSpaces = (index) => {
    const tab = [];
    const start = `${new Date(project.tasks[index].start).getDate()}/${
      new Date(project.tasks[index].start).getMonth() + 1
    }`;
  ...
  const printTask = (index) => {
    ...
    const end = `${new Date(project.tasks[index].end).getDate()}/${
      new Date(project.tasks[index].end).getMonth() + 1
    }`;
    let i = 0;
